...
function Avatar({ name, avatar, size = 'md' }: { name: string; avatar?: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={`${sizes[size]} rounded-full bg-gradient-to-br from-[#E8315B] to-[#BA2749] flex items-center justify-center flex-shrink-0 shadow-sm`}>
        ...
        <div className="flex border-b border-gray-100 px-5">
          {(['orders', 'bookings', 'addresses'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-3 py-2.5 text-sm font-medium capitalize transition-colors border-b-2 -mb-px ${tab === t ? 'border-[#E8315B] text-[#E8315B]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
            >
              {t}
            ...
            !customer.addresses || customer.addresses.length === 0
              ...
              : <div className="space-y-3">
                  {customer.addresses.map((addr, i) => (
                    <div key={i} className={`p-4 rounded-xl border ${addr.isDefault ? 'border-[#E8315B] bg-amber-50/30' : 'border-gray-100 bg-gray-50'}`}>
                      {addr.isDefault && <span className="text-xs font-semibold text-[#E8315B] mb-1 block">Default</span>}
          ...
          <div className="px-5 pt-5 pb-4 border-b border-gray-100 space-y-3 flex-shrink-0">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B] bg-gray-50"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {loading ? (
              <div className="flex items-center justify-center py-16">
                <Loader2 className="w-6 h-6 animate-spin text-[#E8315B]" />
              </div>
            ) : customers.length === 0 ? (
              ...
              return (
                <button
                  ...
                  className={`w-full text-left flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors ${isSelected ? 'bg-amber-50/60 border-l-2 border-[#E8315B]' : ''}`}
                >
