...
const PIE_COLORS = ['#E8315B', '#3B82F6', '#10B981', '#8B5CF6', '#EF4444', '#F59E0B'];
            ...
            {(Object.keys(PERIOD_LABELS) as Period[]).map(p => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                  period === p ? 'bg-[#E8315B] text-white' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
                {p}
              </button>
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-5">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-5">
          <h2 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <BarChart3 className="h-4 w-4 text-[#E8315B]" /> Revenue Trend
          </h2>
          {loading ? (
              ...
              <AreaChart data={trendData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
                <defs>
                  <linearGradient id="revGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%"  stopColor="#E8315B" stopOpacity={0.2} />
                ...
                <Area type="monotone" dataKey="Revenue" stroke="#E8315B" strokeWidth={2} fill="url(#revGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
                ...
                return (
                  <div key={p._id ?? i} className="space-y-1">
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-[#E8315B] rounded-full" style={{ width: `${max > 0 ? Math.round((p.revenue / max) * 100) : 0}%` }} />
                    </div>
                  </div>
                );
              })}
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={categoryData.slice(0, 8)} layout="vertical" margin={{ top: 0, right: 16, bottom: 0, left: 0 }}>
                ...
                <Tooltip formatter={(v: number) => fmt(v)} />
                <Bar dataKey="value" fill="#E8315B" radius={[0, 4, 4, 0]} name="Revenue" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
