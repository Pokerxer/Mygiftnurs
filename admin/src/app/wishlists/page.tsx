    ...
    <div className="group bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg hover:border-[#E8315B]/20 transition-all duration-300">
      <div className="flex items-start gap-4">
        ...
        <button onClick={onView} className="flex-shrink-0">
          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#E8315B] to-[#BA2749] flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
        ...
        <div className="flex-1 min-w-0">
          ...
          <button onClick={onView} className="text-left w-full">
            <p className="font-bold text-gray-900 hover:text-[#E8315B] transition-colors truncate">
        ...
        <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={onView}
            className="p-2.5 text-gray-400 hover:text-[#E8315B] hover:bg-amber-50 rounded-xl transition-all"
            title="View wishlist"
          >
            ...
            <div className="space-y-3">
              {user.wishlist.map((item) => (
                <div key={item._id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/products/${item._id}`} className="font-medium text-gray-900 hover:text-[#E8315B] truncate block">
                      {item.name}
                    </Link>
                    {item.category && (
                    ...
                    {item.variants?.[0]?.price && (
                      <p className="text-sm font-semibold text-[#E8315B] mt-0.5">
              ...
              {stats.topProducts.slice(0, 5).map((product, idx) => (
                ...
                >
                  <div className="relative">
                    ...
                    <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#E8315B] text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                      {product.wishlistCount}
                    </span>
                  </div>
                  <p className="text-xs font-medium text-gray-700 mt-2 truncate group-hover:text-[#E8315B]">
                    {product.name}
                  </p>
                </Link>
              ))}
