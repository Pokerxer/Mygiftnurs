...
function OrderPanel({
  ...
  function printInvoice() {
    ...
    <div class="header">
      <div><h1>MyGiftNurs</h1><div style="color:#666;margin-top:4px">${isRefundRecord ? 'Return/Refund' : 'Tax Invoice'}</div></div>
    ...
    <div class="footer"><p>MyGiftNurs POS · Cashier: ${sale.cashierName}</p></div>
    </body></html>`);
  ...
  function printReceipt() {
    ...
    <p class="c">Thank you for shopping at MyGiftNurs!</p>
    </body></html>`);
    w.document.close(); w.print();
  }
...
export default function SessionDetailPage() {
  ...
  function printZReport() {
    ...
    <h2>MyGiftNurs</h2><p class="c">Z-REPORT</p><p class="c">${new Date().toLocaleString('en-NG')}</p>
    <div class="ln"></div>
  ...
  if (error || !report) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center p-4">
        ...
        <Link href="/pos/dashboard" className="text-sm text-[#E8315B] mt-2 inline-block">← Back to Dashboard</Link>
      </div>
    </div>
  );
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 px-4 py-3 bg-white border-b border-gray-200 flex-shrink-0">
        {[
          ...
          { label: 'Net Revenue', value: formatPrice(report.totalRevenue), icon: <TrendingUp className="w-3.5 h-3.5" />, color: 'text-[#E8315B]' },
              ...
              return (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left flex items-start gap-3 border-l-4 transition ${
                    isActive
                      ? isRefund ? 'bg-orange-50 border-orange-400' : 'bg-amber-50 border-[#E8315B]'
