      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[92vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B]"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2.5 pr-10 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B]"
                  />
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-2 gap-2">
                {(Object.entries(ROLE_CONFIG) as [string, typeof ROLE_CONFIG[string]][]).map(([r, cfg]) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border-2 text-left transition-all ${role === r ? 'border-[#E8315B] bg-[#E8315B]/5' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                    <div className="flex items-center gap-2 mb-0.5">
                      <cfg.icon className={`w-4 h-4 ${role === r ? 'text-[#E8315B]' : 'text-gray-400'}`} />
                      <span className={`text-sm font-medium ${role === r ? 'text-[#E8315B]' : 'text-gray-700'}`}>{cfg.label}</span>
                    </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-semibold text-gray-700 flex items-center gap-1.5">
                    ...
                    {permissions.length > 0 && (
                      <span className="px-1.5 py-0.5 bg-[#E8315B] text-white text-xs rounded-full">{permissions.length}</span>
                    )}
                  </label>
                  <div className="flex items-center gap-1">
                    ...
                    <button type="button" onClick={() => setPermissions(allRoutes.map(r => r.href))} className="text-xs text-[#E8315B] hover:underline px-1">All</button>
                    ...
                    <div key={group.label}>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {group.selected > 0 && <span className="text-xs text-[#E8315B] font-medium">{group.selected}/{group.routes.length}</span>}
                      ...
                      {openGroup === group.label && (
                        <div className="px-2 py-1 bg-white">
                          {group.routes.map(route => {
                            ...
                            return (
                              <label key={route.href} className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${checked ? 'bg-[#E8315B]/5' : 'hover:bg-gray-50'}`}>
                                <input type="checkbox" checked={checked} onChange={() => togglePerm(route.href)} className="rounded border-gray-300 text-[#E8315B] focus:ring-[#E8315B]" />
                                <route.icon className={`w-4 h-4 flex-shrink-0 ${checked ? 'text-[#E8315B]' : 'text-gray-400'}`} />
                ...
                <div className="mt-3 space-y-2">
                  {POS_ACTIONS.map(action => {
                    ...
                    return (
                      <label key={action.key} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${granted ? 'border-[#E8315B] bg-[#E8315B]/5' : 'border-gray-100 hover:border-gray-200 bg-gray-50'}`}>
                        <input type="checkbox" checked={granted} onChange={() => togglePerm(action.key)} className="rounded border-gray-300 text-[#E8315B] focus:ring-[#E8315B]" />
                        ...
                        {granted && <Check className="w-4 h-4 text-[#E8315B] flex-shrink-0" />}
                      </label>
                    );
                  })}
                </div>
          ...
          <div className="flex gap-3 px-6 py-4 border-t flex-shrink-0">
            ...
            <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-[#E8315B] text-white rounded-xl text-sm font-semibold hover:bg-[#BA2749] disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button type="button" onClick={grantAll} className="text-xs text-[#E8315B] hover:underline px-1">All</button>
                  ...
                  {groupsWithSelection.map(group => (
                    <div key={group.label}>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {group.selected > 0 && (
                            <span className="text-xs text-[#E8315B] font-medium">{group.selected}/{group.routes.length}</span>
                          )}
                          <ChevronDown className={`w-3.5 h-3.5 text-gray-400 transition-transform ${openGroup === group.label ? 'rotate-180' : ''}`} />
                        </div>
                      </button>
                        ...
                        <div className="px-2 py-1 bg-white">
                          {group.routes.map(route => {
                            ...
                            return (
                              <label
                                key={route.href}
                                className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-colors ${checked ? 'bg-[#E8315B]/5' : 'hover:bg-gray-50'}`}
                              >
                                <input
                                  type="checkbox"
                                  checked={checked}
                                  onChange={() => togglePerm(route.href)}
                                  className="rounded border-gray-300 text-[#E8315B] focus:ring-[#E8315B]"
                                />
                                <route.icon className={`w-4 h-4 flex-shrink-0 ${checked ? 'text-[#E8315B]' : 'text-gray-400'}`} />
                                <span className={`text-sm flex-1 ${checked ? 'text-gray-900 font-medium' : 'text-gray-600'}`}>{route.label}</span>
                                <code className="text-[10px] text-gray-400">{route.href}</code>
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${
                          granted ? 'border-[#E8315B] bg-[#E8315B]/5' : 'border-gray-100 hover:border-gray-200 bg-gray-50'
                        }`}
                      >
                        <input
                          type="checkbox"
                ...
                {showResetPin && (
                  <div className="p-4 space-y-3 bg-white">
                    ...
                    <input
                      ...
                      maxLength={8}
                      className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B] tracking-widest font-mono"
                    />
                    {pinMsg && (
          ...
          <div className="flex gap-3 px-6 py-4 border-t flex-shrink-0">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 bg-[#E8315B] text-white rounded-xl text-sm font-semibold hover:bg-[#BA2749] disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading && <Loader2 className="w-4 h-4 animate-spin" />}
              Save Changes
            </button>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <ShieldCheck className="w-6 h-6 text-[#E8315B]" /> User Access Management
          </h1>
          <p className="text-gray-500 mt-0.5">Control who can access the admin panel and what they can do</p>
        </div>
        <button
          onClick={() => setCreating(true)}
          className="flex items-center gap-2 px-4 py-2.5 bg-[#E8315B] text-white rounded-xl text-sm font-semibold hover:bg-[#BA2749] transition-colors flex-shrink-0"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B]"
          />
        </div>
          ...
          <div className="flex rounded-xl border border-gray-200 overflow-hidden">
            {[{ v: '', l: 'All' }, { v: 'admin', l: 'Admin' }, { v: 'staff', l: 'Staff' }, { v: 'therapist', l: 'Therapist' }, { v: 'customer', l: 'Customer' }].map(f => (
              <button
                ...
                className={`px-3 py-2 text-sm font-medium transition-colors whitespace-nowrap ${roleFilter === f.v ? 'bg-[#E8315B] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
              >
                {f.l}
              </button>
            ))}
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        className="p-2 text-gray-400 hover:text-[#E8315B] hover:bg-[#E8315B]/10 rounded-lg transition"
                        title="Edit access"
                      >
