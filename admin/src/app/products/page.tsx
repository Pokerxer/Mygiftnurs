        ...
        <div className="mb-8 flex items-center justify-between animate-fade-in">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 flex items-center gap-3">
              <span className="w-2 h-8 bg-[#E8315B] rounded-full" />
              Products
            </h1>
          ...
          <div className="flex gap-2">
            {isAdmin && (
              <Link
                href="/products/new"
                className="flex items-center gap-2 px-5 py-2.5 bg-[#E8315B] text-white rounded-xl font-medium hover:bg-[#BA2749] transition-all shadow-lg shadow-[#E8315B]/20"
              >
        ...
        <div className="mb-5 space-y-3">
          ...
          <div className="flex gap-3 items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B] bg-gray-50/50 hover:bg-gray-50 transition-all"
              />
            </div>
            <div className="flex-shrink-0 flex items-center gap-1 border border-gray-200 rounded-xl p-1 bg-white">
              <button
                onClick={() => setView('list')}
                className={`p-1.5 rounded-lg transition-all ${view === 'list' ? 'bg-[#E8315B] text-white shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                title="List view"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded-lg transition-all ${view === 'grid' ? 'bg-[#E8315B] text-white shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                title="Grid view"
              >
          ...
          <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
              ...
              <button
                ...
                className={`flex-none px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                  statusFilter === f.value
                    ? 'bg-[#E8315B] text-white shadow-md shadow-[#E8315B]/20'
            ...
            {categories.length > 0 && (
              <div className="relative flex-none">
                <select
                  ...
                  className="appearance-none px-4 py-2 pr-8 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 bg-white focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 cursor-pointer min-w-[140px]"
                >
            ...
            <input
              ...
              className="flex-none px-3 py-2 rounded-xl text-sm border border-gray-200 text-gray-600 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#E8315B]/20 focus:border-[#E8315B] w-36 bg-white"
            />
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-semibold text-gray-900 flex items-center gap-2">
              <Package className="h-5 w-5 text-[#E8315B]" />
              Product Catalogue
            </h2>
          ...
          {loading ? (
            <div className="py-16 flex justify-center">
              <Loader2 className="h-6 w-6 animate-spin text-[#E8315B]" />
            </div>
          ) : products.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <Link
                href="/products/new"
                className="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-[#E8315B] text-white rounded-xl text-sm font-medium hover:bg-[#BA2749] transition-colors"
              >
                <Plus className="h-4 w-4" />
                Add Product
              </Link>
                      ...
                      <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <Link
                                ...
                                className="font-medium text-gray-900 hover:text-[#E8315B] transition-colors line-clamp-1"
                              >
                                {product.name}
                              </Link>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {isAdmin && (
                              <Link href={`/products/${product._id}/edit`} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-[#E8315B] transition-colors" title="Edit">
            ...
            <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {sortedProducts.map(product => {
                ...
                return (
                  <div
                    key={product._id}
                    className="group relative bg-white rounded-2xl border border-gray-100 hover:border-[#E8315B]/30 hover:shadow-lg transition-all duration-200 overflow-hidden flex flex-col"
                  >
                    {/* Image */}
                    <div className="relative aspect-square bg-gray-50 overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <Link
                          ...
                          className="p-2 bg-white rounded-xl text-gray-700 hover:text-[#E8315B] shadow-md transition-colors"
                          title="View"
                        >
                          <Eye className="h-4 w-4" />
                        </Link>
                    ...
                    <div className="p-3 flex flex-col gap-1 flex-1">
                      <Link href={`/products/${product._id}/edit`} className="text-sm font-semibold text-gray-900 hover:text-[#E8315B] transition-colors leading-tight line-clamp-2">
                        {product.name}
                      </Link>
                      ...
                      <div className="mt-auto pt-2 flex items-center justify-between">
                        <span className="text-sm font-bold text-[#E8315B]">{formatPrice(getMinPrice(product))}</span>
