      ...
      <aside className={cn(
        ...
        <div className="flex-shrink-0 h-16 flex items-center justify-between border-b border-gray-100 px-4 lg:px-6">
          ...
          >
            {logoFailed ? (
              <div>
                <span className="font-bold text-lg bg-gradient-to-r from-[#E8315B] via-[#F2C572] to-[#E8315B] bg-clip-text text-transparent">MyGiftNurs</span>
                <span className="block text-[10px] text-[#E8315B] font-medium -mt-1">Admin</span>
              </div>
            ) : (
              <>
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-[#E8315B] to-[#BA2749] shadow-lg shadow-[#E8315B]/20 overflow-hidden flex-shrink-0">
                  <img
                    src="/logo.png"
                    alt="MyGiftNurs"
                ...
                <span className="block text-[10px] text-[#E8315B] font-medium">Admin</span>
              </>
            )}
          </Link>
          <button
        ...
        <div className="lg:hidden p-4 border-b border-gray-100">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-[#E8315B] to-[#BA2749] flex items-center justify-center shadow-md flex-shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 capitalize flex items-center gap-1">
                {userRole === 'admin'
                  ? <><ShieldCheck className="w-3 h-3 text-brand-pink" /> Admin</>
                  : permCount > 0
        ...
        <nav className="space-y-0.5 p-3 overflow-y-auto flex-1 custom-scrollbar">
          {filteredNav.map((item, index) => {
                ...
                <div key={item.name}>
                  <button
                    ...
                    className={cn(
                      ...
                      isProductsPage
                        ? 'bg-gradient-to-r from-[#E8315B]/10 to-[#E8315B]/5 text-[#E8315B]'
                        : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <item.icon className={cn('h-5 w-5', isProductsPage ? 'text-[#E8315B]' : '')} />
                      {item.name}
                    </div>
            ...
            return (
              ...
              >
                <item.icon className={cn('h-5 w-5', isActive ? 'text-[#E8315B]' : '')} />
                {item.name}
                {isActive && <div className="ml-auto h-1.5 w-1.5 rounded-full bg-[#E8315B]" />}
              </Link>
            );
          })}
        </nav>
        ...
        <div className="hidden lg:block p-4 border-t border-gray-100 bg-white flex-shrink-0 space-y-3">
          <div className="rounded-xl bg-gradient-to-r from-gray-900 to-gray-800 p-3.5 text-white">
            <div className="flex items-center justify-between mb-0.5">
              ...
              {userRole === 'admin'
                ? <ShieldCheck className="w-3.5 h-3.5 text-brand-pink-pale flex-shrink-0" />
                : permCount > 0
