...
const { uploadImage, deleteImage, deleteImages, getOptimizedUrl, CLOUDINARY_FOLDER } = require('../utils/cloudinary');
...
exports.createProduct = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      const uploadPromises = req.files.map(file => uploadImage(file.path, CLOUDINARY_FOLDER));
