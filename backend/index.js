...
const corsOrigin = process.env.CORS_ORIGIN?.split(',').map(o => o.trim()) || [
  ...
  'http://localhost:3002',
  'http://localhost:7002',
  'https://admin.mygiftnursemporium.com',
  'https://www.mygiftnursemporium.com'
];
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/mygiftnurs';
...
app.post('/api/seed/admin', async (req, res) => {
  ...
  try {
    ...
    const admin = new User({
      name: name || 'Admin',
      email: email || 'admin@mygiftnurs.com',
...
app.get('/', (req, res) => {
  res.json({ 
    message: 'MyGiftNurs Backend API', 
    version: '1.0.0',
    endpoints: [
      '/api/store/products',
...
if (process.env.NODE_ENV !== 'production') {
  app.listen(PORT, () => {
    console.log(`MyGiftNurs Backend API running on http://localhost:${PORT}`);
  });
}
